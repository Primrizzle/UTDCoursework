// This C++ program demonstrates Inter-Process Communication (IPC) using pipes and the fork() system call to create a parent-child 
// process relationship. The parent process sends a message to the child to retrieve a quote from a list, and the child process reads 
// the request, selects a random quote from a file, and sends it back to the parent through the pipe. The program continues this 
// communication for a specified number of times before terminating.
